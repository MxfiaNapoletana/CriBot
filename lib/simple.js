      ...
      get() {
        ...
        text: {
            get() {
                ...
                // 🔹 PRIORITÀ 1: Bottoni interattivi moderni (buttonsResponseMessage)
                ...
                // 🔹 PRIORITÀ 2: Bottoni nativi (interactiveResponseMessage)
                if (this.message?.interactiveResponseMessage) {
                    const nativeFlow = this.message.interactiveResponseMessage.nativeFlowResponseMessage
                    if (nativeFlow?.paramsJson) {
                        try {
                            const params = JSON.parse(nativeFlow.paramsJson)
                            // Restituisci l'ID del bottone premuto
                            if (params.id) return params.id
                        } catch (e) {
                            console.error('Errore parsing bottone nativo:', e)
                        }
                    }
                }
                
                // 🔹 PRIORITÀ 3: Testo normale
