...
async function handler(m, { conn }) {
    ...
    try {
        ...
        if (cachedResult && (Date.now() - cachedResult.timestamp < cacheTime)) {
            return m.reply(cachedResult.report, null, { 
                cached: true,
                mentions: cachedResult.mentions 
            });
        }
        ...
        const texts = [];
        const senderStats = {};
        const senderIds = {}; // Mappa nome -> ID JID
...
FORMATO JSON RICHIESTO:
{
  ...
  "insights_avanzati": ["Insight 1", "Insight 2", "Insight 3"]
        ...
        // 5. Costruzione Report con Menzioni
        const mentionsList = stats.topSenders.map(sender => senderIds[sender.name]).filter(Boolean);

        let report = `╭━━━『 📊 *ANALISI GRUPPO* 』━━━╮\n`;
        report += `│\n`;
        report += `│ 📱 *Gruppo:* ${groupName}\n`;
        report += `│ 📨 *Messaggi analizzati:* ${stats.messageCount}\n`;
        report += `│ 👥 *Utenti attivi:* ${Object.keys(senderStats).length}\n`;
        report += `│\n`;
        report += `╰━━━━━━━━━━━━━━━━━━━━╯\n\n`;
        ...
        if (aiAnalysis) {
            report += `┌─ 『 💭 *VALUTAZIONE* 』\n`;
            report += `│\n`;
            report += `│ ${aiAnalysis.valutazione_generale}\n`;
            report += `│\n`;
            report += `└─────────────────\n\n`;
            ...
            if (aiAnalysis.riassunto_messaggi) {
                report += `┌─ 『 📝 *CONTENUTI* 』\n`;
                report += `│\n`;
                report += `│ ${aiAnalysis.riassunto_messaggi}\n`;
                report += `│\n`;
                report += `└─────────────────\n\n`;
            }
        }
        
        report += `┌─ 『 🎭 *STATO EMOTIVO* 』\n`;
        report += `│\n`;
        report += `│ ${emotionsConfig[dominantEmotion].description}\n`;
        report += `│\n`;
        report += `└─────────────────\n\n`;
        ...
        if (stats.topSenders.length > 0) {
            ...
            stats.topSenders.forEach((sender, index) => {
                ...
                const senderJid = senderIds[sender.name];
                report += `│ ${medal} @${senderJid.split('@')[0]}\n`;
                report += `│    ├ Messaggi: ${sender.messageCount}\n`;
                report += `│    └ Parole: ${sender.wordCount}\n`;
                if (index < stats.topSenders.length - 1) report += `│\n`;
            });
            report += `│\n`;
            report += `└─────────────────\n\n`;
        }
        ...
        report += `┌─ 『 📈 *STATISTICHE* 』\n`;
        report += `│\n`;
        report += `│ 💬 *Parole totali:* ${stats.totalWords}\n`;
        report += `│ 😄 *Emoji usate:* ${stats.emojiCount}\n`;
        if (stats.topWords.length > 0) {
            report += `│ 🔤 *Parole top:* ${stats.topWords.slice(0, 3).join(', ')}\n`;
        }
        report += `│\n`;
        report += `└─────────────────\n`;
        ...
        if (aiAnalysis?.insights_avanzati?.length) {
            report += `\n┌─ 『 💡 *INSIGHTS* 』\n`;
            report += `│\n`;
            aiAnalysis.insights_avanzati.forEach((insight, idx) => {
        ...
        report += `\n⏱️ _Analisi completata in ${analysisTime}s_`;
        
        global.moodCache.set(cacheKey, { 
            report, 
            timestamp: Date.now(),
            mentions: mentionsList 
        });
        
        await m.reply(report, null, { mentions: mentionsList });
        await m.react('✅');
