...
let handler = async (m, { conn, args, participants, isAdmin, isBotAdmin }) => {
  ...
  if (m.text?.toLowerCase() === '.indizio') {
    ...
    // Controlla se l'utente ha già usato l'indizio
    if (game.indizioUsato?.[m.sender]) {
      return m.reply('❌ *Hai già usato il tuo indizio!*\n\n🤔 *Prova a indovinare!*')
    }
    
    // Segna che l'utente ha usato l'indizio
    if (!game.indizioUsato) game.indizioUsato = {}
    game.indizioUsato[m.sender] = true
    
    const primaLettera = game.rispostaOriginale[0].toUpperCase()
    const numeroLettere = game.rispostaOriginale.length
    const ultimaLettera = game.rispostaOriginale[game.rispostaOriginale.length - 1].toUpperCase()
    
    const indizioMessage = `💡 *INDIZIO RICHIESTO!*

📝 *Informazioni sulla nazione:*
   • Inizia con: *"${primaLettera}"*
   • Finisce con: *"${ultimaLettera}"*
   • Numero di lettere: *${numeroLettere}*

🎯 *Ora prova a indovinare!*

> \`cri bot\``
    
    await conn.reply(m.chat, indizioMessage, m)
    return
  }

  // Comando .skipbandiera
  if (m.text?.toLowerCase() === '.skipbandiera') {
    ...
    // Messaggio con bottone "Gioca Ancora"
    const skipMessage = `🛑 *Gioco delle bandiere interrotto dall'admin*\n✨ La risposta era: *${global.bandieraGame[m.chat].rispostaOriginale}*\n\n> \`cri bot\``
  ...
  try {
    ...
    global.bandieraGame[m.chat] = {
      ...
      timeout: setTimeout(async () => {
        if (global.bandieraGame?.[m.chat]) {
          // Messaggio timeout con bottone "Gioca Ancora"
          const timeoutMessage = `⏳ *Tempo scaduto!*\n\n🌍 *La risposta era:* *${scelta.nome}*\n\n> \`cri bot\``
...
handler.before = async (m, { conn }) => {
    ...
    if (isAnswerCorrect(userAnswer, correctAnswer)) {
        ...
        // Forza la scrittura del database
...
> \`cri bot\``
        ...
        // Invia il messaggio di congratulazioni CON il bottone "Gioca Ancora"
        const buttons = [
            { 
                buttonId: '.bandiera', 
                buttonText: { displayText: '🔄 Gioca Ancora' }, 
        ...
        await conn.sendMessage(chat, buttonMessage, { quoted: m })
        ...
        if (tentativiRimasti === 2) {
            // Prima risposta sbagliata - mostra bottone indizio
            const hasUsedIndizio = game.indizioUsato?.[m.sender]
            
            if (hasUsedIndizio) {
                // Se ha già usato l'indizio, non mostrare il bottone
                await conn.reply(chat, `❌ *Risposta errata!*

📝 *Tentativi rimasti:* 2
🤔 *Pensa bene alla tua prossima risposta!*`, m)
            } else {
                // Mostra il bottone indizio
                const buttons = [
                    { 
                        buttonId: '.indizio', 
                        buttonText: { displayText: '💡 Richiedi Indizio' }, 
                        type: 1 
                    }
                ]
...
handler.help = ['bandiera', 'indizio', 'skipbandiera']
handler.tags = ['giochi']
handler.command = /^(bandiera|indizio|skipbandiera)$/i
handler.group = true
